/**
 * IAPService Practical Error Scenario Tests
 * 
 * 실제 프로덕션에서 발생할 수 있는 에러 시나리오들을 테스트합니다.
 * Tests real-world error scenarios that can occur in production.
 */
...
const mockGetAvailablePurchases = require('react-native-iap').getAvailablePurchases;
...
describe('IAPService - Practical Error Scenarios', () => {
  ...
  describe('Network Connection Issues', () => {
    ...
    test('should handle intermittent connectivity issues', async () => {
      ...
      mockGetAvailablePurchases.mockRejectedValue(new Error('Network temporarily unavailable'));
  ...
  describe('Purchase Validation Failures', () => {
    test('should handle server receipt validation timeout', async () => {
      ...
      jest.spyOn(IAPService as any, 'validatePurchase').mockImplementation(
        () => new Promise((_, reject) => 
      ...
      // Should handle validation timeout gracefully (fallback to free)
    ...
    test('should handle corrupted receipt data', async () => {
      ...
      jest.spyOn(IAPService as any, 'validatePurchase').mockResolvedValue(false);
    ...
    test('should handle expired subscription validation', async () => {
      ...
      // Should handle expired subscription correctly
    });
  });
  ...
  describe('Purchase Flow Edge Cases', () => {
    test('should handle user cancellation during purchase', async () => {
      ...
      const result = await IAPService.purchaseSubscription(IAP_PRODUCT_IDS.PRO_MONTHLY);
    ...
    test('should handle duplicate purchase attempts', async () => {
      ...
      const processedPurchases = (IAPService as any).processedPurchases as Set<string>;
  ...
  describe('Concurrent Operations', () => {
    test('should handle concurrent initialization calls', async () => {
      ...
      const promises = Array(10).fill(null).map(() => IAPService.initialize());
  ...
  describe('Production Environment Simulation', () => {
    ...
    test('should handle TestFlight environment', async () => {
      ...
      jest.spyOn(IAPService as any, 'determineTestEnvironment').mockReturnValue(true);
...
/**
 * These error scenario tests cover:
 * 
 * ✅ Network Issues:
 *   - Connection timeouts
 ...
 *   - TestFlight behavior
 * 
 * These tests help ensure robust error handling in real-world scenarios.
 */