...
describe('StorageService', () => {
  ...
  describe('Favorites Management', () => {
    ...
    test('should handle addFavorite errors gracefully', async () => {
      ...
      // The error is actually logged from getFavorites() first
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Error loading favorites:',
        expect.any(Error)
      );
  ...
  describe('History Management', () => {
    const mockHistoryItem: Omit<HistoryItem, 'id' | 'searchedAt'> = {
      ...
      searchedData: [{ lng: 'ko', text: '안녕하세요' }],
    };
    ...
    test('should remove duplicate entries when adding to history', async () => {
      ...
      // Check that setItem was called with the correct key and parse the JSON to compare objects
      expect(mockAsyncStorage.setItem).toHaveBeenCalledWith(
        'dictionary_history',
        expect.any(String)
      );
      
      const actualStoredHistory = JSON.parse(
        mockAsyncStorage.setItem.mock.calls[0][1]
      );
      expect(actualStoredHistory).toEqual(expectedHistory);
    ...
    test('should limit history to 100 items', async () => {
      ...
      await StorageService.addToHistory({
        ...
        sourceLanguage: 'en',
        targetLanguage: 'ko',
        translatedText: '새로운 단어',
        searchedData: [{ lng: 'ko', text: '새로운 단어' }],
      });
  ...
  describe('Language Settings', () => {
    ...
    test('should handle language settings errors gracefully', async () => {
      ...
      expect(languages).toEqual([]);
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Error loading selected languages:',
        expect.any(Error)
      );
