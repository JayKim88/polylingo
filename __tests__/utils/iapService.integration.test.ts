...
describe('IAPService Integration Tests', () => {
  beforeEach(() => {
    ...
    jest.spyOn(IAPService as any, 'handleSuccessfulPurchaseQuietly').mockResolvedValue(undefined);
    jest.spyOn(IAPService as any, 'setSubscriptionFreeWithPreserve').mockResolvedValue(undefined);
  });
  ...
  describe('checkSubscriptionStatusAndUpdate', () => {
    ...
    test('handles multiple purchases - selects latest', async () => {
      ...
      const newPurchase = createPurchase(IAP_PRODUCT_IDS.PREMIUM_YEARLY, 'tx_new');
      newPurchase.transactionDate = Date.now() + 1000; // Make it newer
      
      mockIAP.getAvailablePurchases.mockResolvedValue([oldPurchase, newPurchase]);
    ...
    test('handles validation failure - falls back to free', async () => {
      const purchase = createPurchase(IAP_PRODUCT_IDS.PRO_MONTHLY, 'tx_invalid');
      mockIAP.getAvailablePurchases.mockResolvedValue([purchase]);
    ...
    test('handles API errors - sets free plan', async () => {
      mockIAP.getAvailablePurchases.mockRejectedValue(new Error('Apple ID not logged in'));
  ...
  describe('Error Recovery', () => {
    test('recovers from sync errors', async () => {
      const purchase = createPurchase(IAP_PRODUCT_IDS.PREMIUM_YEARLY, 'tx_error');
      mockIAP.getAvailablePurchases.mockResolvedValue([purchase]);
      mockSubscriptionService.setSubscription.mockRejectedValue(new Error('Server sync failed'));
